...
int BipartiteGraph::count_vertices_a() {
  return this->n0;
}

int BipartiteGraph::count_vertices_b() {
  return this->n1;
}

void BipartiteGraph::print_b_neighbours() {
  for (int b = this->n0; b < this->n0 + this->n1; b++) {
    std::cout << "N(" << b + 1 << "): ";
    for (int a : this->neighborhood(b)) {
      std::cout << a + 1 << " ";
    }
    std::cout << std::endl;
  }
...
std::vector<std::vector<int>> BipartiteGraph::build_crossing_matrix() {
  std::vector<std::vector<int>> cm(this->n1, std::vector<int>(this->n1, 0));
  for (int b1 = 0; b1 < this->n1; b1++) {
    for (int b2 = 0; b2 < this->n1; b2++) {
      if (b1 == b2) continue;
      int crossings = 0;
      for (int a1 : this->neighborhood(b1 + this->n0))
        for (int a2 : this->neighborhood(b2 + this->n0))
          if (a2 < a1) crossings++;
      cm[b1][b2] = crossings;
    }
  }
