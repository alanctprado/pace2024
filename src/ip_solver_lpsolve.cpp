...
int LPSolveSolver::simple()
{
  ...
  for (auto vertex : m_instance) {
    m_vertex_map[vertex.first] = vertex;
  }
  ...
  for (int k = 1; k <= number_vars; k++)
  {
    auto [i, j] = orientable_pairs[k - 1];
    {
      ...
      c[k] = m_oracle.getCrossings(_vi, _vj);
    }
  }
  ...
  for (auto [i, _] : m_instance)
  {
    int count_successors = 0;
    for (auto [j, __] : m_instance)
    {
      if (i == j)
      {
        continue;
  ...
  {
    m_order.push_back(m_vertex_map[it->second]);
  }
