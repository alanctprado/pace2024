...
bool Preprocessing::lmr_reduction(Oracle::SubProblem &instance) {
  ...
  auto get_left = [&] (auto&& self, int l, int r, int p, int a, int b) -> int {
	...
	if (r == l) return l;
	int m = (l+r)/2;
	int x = self(self, l, m, 2*p, a, b);
	if (x != -1) return x;
	return self(self, m+1, r, 2*p+1, a, b);
  };
  ...
  for (auto [l, r] : intervals) {
      upd(upd, 0, m-1, 1, l, 0, 1);
      upd(upd, 0, m-1, 1, r, 1, 1);
  }
  ...
  for (int i = 0; i < n; ++i) {
      ...
      upd(upd, 0, m-1, 1, intervals[i].first, 0, -1);
      upd(upd, 0, m-1, 1, intervals[i].second, 1, -1);
      int l_max = get_right(get_right, 0, m-1, 1, 0, intervals[i].first);
      int r_min = get_left(get_left, 0, m-1, 1, intervals[i].second, m-1);
      bool deu_certo = 1;
